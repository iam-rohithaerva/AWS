...
- [5. Why each AWS service](#5-why-each-aws-service)
  - Services: [Amazon Route 53](#amazon-route-53) · [Amazon CloudFront](#amazon-cloudfront) · [AWS WAF](#aws-waf) · [AWS Certificate Manager (ACM)](#aws-certificate-manager-acm) · [AWS Shield Standard](#aws-shield-standard) · [Amazon S3](#amazon-s3) · [Amazon Cognito](#amazon-cognito) · [Amazon API Gateway](#amazon-api-gateway) · [AWS Lambda](#aws-lambda) · [Amazon DynamoDB (and DynamoDB Streams)](#amazon-dynamodb-and-dynamodb-streams) · [Amazon EventBridge (event buses and rules)](#amazon-eventbridge-event-buses-and-rules) · [Amazon EventBridge Scheduler](#amazon-eventbridge-scheduler) · [AWS Step Functions](#aws-step-functions) · [Amazon Textract](#amazon-textract) · [Amazon SQS](#amazon-sqs) · [Amazon SNS](#amazon-sns) · [Amazon SES](#amazon-ses) · [AWS IAM](#aws-iam) · [AWS KMS](#aws-kms) · [AWS Secrets Manager](#aws-secrets-manager) · [AWS Backup](#aws-backup) · [Amazon CloudWatch](#amazon-cloudwatch) · [AWS X-Ray](#aws-x-ray) · [AWS CloudTrail](#aws-cloudtrail)
  - [Key decisions](#key-decisions)
- [5A. Key topics](#5a-key-topics)
  - [Synchronous vs asynchronous processing (sync vs async)](#synchronous-vs-asynchronous-processing-sync-vs-async)
  - [Lambda vs ECS/Fargate and the cost crossover point](#lambda-vs-ecsfargate-and-the-cost-crossover-point)
  - [DynamoDB single-table design and access patterns](#dynamodb-single-table-design-and-access-patterns)
  - [Handling month-end spikes](#handling-month-end-spikes)
  - [Protecting a fragile downstream](#protecting-a-fragile-downstream)
...
- [11. Failure scenarios](#11-failure-scenarios)
  - [Failure 1: Bad Lambda deploy (a new version with a bug)](#failure-1-bad-lambda-deploy-a-new-version-with-a-bug)
  - [Failure 2: Month-end throttling (Lambda concurrency or API 429)](#failure-2-month-end-throttling-lambda-concurrency-or-api-429)
  - [Failure 3: Customer ERP down (payout queue backlog)](#failure-3-customer-erp-down-payout-queue-backlog)
  - [Failure 4: Textract throttling or outage](#failure-4-textract-throttling-or-outage)
  - [Failure 5: Stream Lambda poison record (events stop)](#failure-5-stream-lambda-poison-record-events-stop)
  - [Failure 6: DynamoDB hot partition throttling](#failure-6-dynamodb-hot-partition-throttling)
  - [Failure 7: Approval never comes (task token stuck)](#failure-7-approval-never-comes-task-token-stuck)
  - [Failure 8: The whole eu-west-1 region is down](#failure-8-the-whole-eu-west-1-region-is-down)
...
- [14. Deep-dive questions and answers](#14-deep-dive-questions-and-answers)
  - [Q1. Walk me through what happens when an employee uploads a receipt until they get reimbursed.](#q1-walk-me-through-what-happens-when-an-employee-uploads-a-receipt-until-they-get-reimbursed)
  - [Q2. Why Lambda instead of ECS on Fargate? At 1,500 req/s steady, isn't Fargate cheaper?](#q2-why-lambda-instead-of-ecs-on-fargate-at-1500-reqs-steady-isnt-fargate-cheaper)
  - [Q3. Why DynamoDB instead of Aurora Serverless v2?](#q3-why-dynamodb-instead-of-aurora-serverless-v2)
  - [Q4. How does your single-table design serve the approver inbox?](#q4-how-does-your-single-table-design-serve-the-approver-inbox)
  - [Q5. Month-end traffic is 10x. What breaks first and how do you prepare?](#q5-month-end-traffic-is-10x-what-breaks-first-and-how-do-you-prepare)
  - [Q6. How do you protect a customer ERP that accepts only a few calls per second?](#q6-how-do-you-protect-a-customer-erp-that-accepts-only-a-few-calls-per-second)
  - [Q7. Why Step Functions Standard instead of Express? Where would you use Express?](#q7-why-step-functions-standard-instead-of-express-where-would-you-use-express)
  - [Q8. How does the task token approval work, and how do you secure it?](#q8-how-does-the-task-token-approval-work-and-how-do-you-secure-it)
  - [Q9. Why SQS for ERP calls instead of sending events straight from EventBridge to a Lambda?](#q9-why-sqs-for-erp-calls-instead-of-sending-events-straight-from-eventbridge-to-a-lambda)
  - [Q10. Your Lambdas are not in a VPC. When would you put a Lambda in a VPC?](#q10-your-lambdas-are-not-in-a-vpc-when-would-you-put-a-lambda-in-a-vpc)
  - [Q11. SQS is at-least-once. How do you make sure a payout is never paid twice?](#q11-sqs-is-at-least-once-how-do-you-make-sure-a-payout-is-never-paid-twice)
  - [Q12. How do you stop someone from calling API Gateway directly and skipping CloudFront and WAF?](#q12-how-do-you-stop-someone-from-calling-api-gateway-directly-and-skipping-cloudfront-and-waf)
  - [Q13. Why REST API instead of HTTP API?](#q13-why-rest-api-instead-of-http-api)
  - [Q14. eu-west-1 goes down. Walk me through recovery. Why is an RPO of 24 hours acceptable?](#q14-eu-west-1-goes-down-walk-me-through-recovery-why-is-an-rpo-of-24-hours-acceptable)
  - [Q15. How do you deal with cold starts?](#q15-how-do-you-deal-with-cold-starts)
  - [Q16. How do you isolate tenants in a shared DynamoDB table?](#q16-how-do-you-isolate-tenants-in-a-shared-dynamodb-table)
  - [Q17. Why publish domain events from DynamoDB Streams instead of from the API Lambda?](#q17-why-publish-domain-events-from-dynamodb-streams-instead-of-from-the-api-lambda)
  - [Q18. What would you do differently if you built it again?](#q18-what-would-you-do-differently-if-you-built-it-again)
- [Glossary](#glossary)
