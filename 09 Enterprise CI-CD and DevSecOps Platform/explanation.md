...
- [5. Why each AWS service](#5-why-each-aws-service)
  - Services: [AWS CodeConnections](#aws-codeconnections) · [AWS CodePipeline (V2)](#aws-codepipeline-v2) · [AWS CodeBuild](#aws-codebuild) · [Amazon VPC and VPC endpoints](#amazon-vpc-and-vpc-endpoints) · [Amazon S3](#amazon-s3) · [Amazon ECR](#amazon-ecr) · [Amazon Inspector](#amazon-inspector) · [AWS KMS and AWS Signer (signing key, KMS CMKs)](#aws-kms-and-aws-signer-signing-key-kms-cmks) · [AWS Secrets Manager](#aws-secrets-manager) · [AWS IAM (cross-account roles, OIDC)](#aws-iam-cross-account-roles-oidc) · [IAM Identity Center](#iam-identity-center) · [AWS Organizations](#aws-organizations) · [AWS CodeDeploy](#aws-codedeploy) · [Amazon ECS on Fargate](#amazon-ecs-on-fargate) · [Application Load Balancer (ALB)](#application-load-balancer-alb) · [Amazon EKS and EC2 nodes](#amazon-eks-and-ec2-nodes) · [AWS Lambda](#aws-lambda) · [Amazon Managed Service for Prometheus](#amazon-managed-service-for-prometheus) · [Amazon DynamoDB (Terraform lock)](#amazon-dynamodb-terraform-lock) · [Amazon EventBridge](#amazon-eventbridge) · [Amazon CloudWatch](#amazon-cloudwatch) · [Amazon SNS](#amazon-sns) · [Amazon Q Developer in chat applications](#amazon-q-developer-in-chat-applications) · [AWS CloudTrail](#aws-cloudtrail) · [Argo CD and Argo Rollouts (open source, running in EKS)](#argo-cd-and-argo-rollouts-open-source-running-in-eks) · [Kyverno (open source admission controller)](#kyverno-open-source-admission-controller) · [Trivy (open source scanner)](#trivy-open-source-scanner)
  - [Key decisions](#key-decisions)
- [5A. Key topics](#5a-key-topics)
  - [CI/CD flow end to end (from start to finish)](#cicd-flow-end-to-end-from-start-to-finish)
  - [Infrastructure as Code (Terraform structure, state, modules, multi-account)](#infrastructure-as-code-terraform-structure-state-modules-multi-account)
  - [Blue/green deployment (ECS, CodeDeploy)](#bluegreen-deployment-ecs-codedeploy)
  - [Canary deployment (EKS Argo Rollouts, Lambda CodeDeploy)](#canary-deployment-eks-argo-rollouts-lambda-codedeploy)
  - [Rollback (application, infrastructure, database migrations)](#rollback-application-infrastructure-database-migrations)
  - [Container security (base images, non-root, scanning, signing, admission, runtime)](#container-security-base-images-non-root-scanning-signing-admission-runtime)
  - [Secrets management (in the pipeline, at runtime)](#secrets-management-in-the-pipeline-at-runtime)
  - [Image scanning (where, what blocks, CVE exceptions)](#image-scanning-where-what-blocks-cve-exceptions)
  - [Multi-account deployment model and separation of duties](#multi-account-deployment-model-and-separation-of-duties)
...
- [11. Failure scenarios](#11-failure-scenarios)
  - [Failure 1: Bad release in EKS canary](#failure-1-bad-release-in-eks-canary)
  - [Failure 2: ECS green tasks fail after traffic switch](#failure-2-ecs-green-tasks-fail-after-traffic-switch)
  - [Failure 3: Critical CVE published for images already in prod](#failure-3-critical-cve-published-for-images-already-in-prod)
  - [Failure 4: CodeBuild can't reach ECR or Secrets Manager (endpoint/DNS problem)](#failure-4-codebuild-cant-reach-ecr-or-secrets-manager-endpointdns-problem)
  - [Failure 5: Terraform state lock stuck / concurrent apply](#failure-5-terraform-state-lock-stuck--concurrent-apply)
  - [Failure 6: Someone tries to bypass the pipeline (compromised credential)](#failure-6-someone-tries-to-bypass-the-pipeline-compromised-credential)
  - [Failure 7: Migration breaks rollback (expand/contract not followed)](#failure-7-migration-breaks-rollback-expandcontract-not-followed)
  - [Failure 8: us-east-1 region outage](#failure-8-us-east-1-region-outage)
  - [Failure 9: Kyverno pods down (admission webhook fail)](#failure-9-kyverno-pods-down-admission-webhook-fail)
  - [Failure 10: CodeBuild queue backlog (quota hit)](#failure-10-codebuild-queue-backlog-quota-hit)
...
- [14. Deep-dive questions and answers](#14-deep-dive-questions-and-answers)
  - [Q1. Walk me through what happens from a developer's commit to production.](#q1-walk-me-through-what-happens-from-a-developers-commit-to-production)
  - [Q2. Why CodePipeline and CodeBuild instead of GitHub Actions or Jenkins?](#q2-why-codepipeline-and-codebuild-instead-of-github-actions-or-jenkins)
  - [Q3. Why GitOps with Argo CD for EKS, but CodeDeploy for ECS and Lambda? Why not one tool?](#q3-why-gitops-with-argo-cd-for-eks-but-codedeploy-for-ecs-and-lambda-why-not-one-tool)
  - [Q4. How does the pipeline deploy into another AWS account? What IAM and KMS setup is needed?](#q4-how-does-the-pipeline-deploy-into-another-aws-account-what-iam-and-kms-setup-is-needed)
  - [Q5. How do you make sure only images built by your pipeline run in prod?](#q5-how-do-you-make-sure-only-images-built-by-your-pipeline-run-in-prod)
  - [Q6. A critical CVE is published for an image that is already running in prod. What happens?](#q6-a-critical-cve-is-published-for-an-image-that-is-already-running-in-prod-what-happens)
  - [Q7. What exactly blocks a release, and how do you handle a CVE exception?](#q7-what-exactly-blocks-a-release-and-how-do-you-handle-a-cve-exception)
  - [Q8. Blue/green or canary: when do you pick which?](#q8-bluegreen-or-canary-when-do-you-pick-which)
  - [Q9. How does Argo Rollouts decide to roll back? What if the service has low traffic?](#q9-how-does-argo-rollouts-decide-to-roll-back-what-if-the-service-has-low-traffic)
  - [Q10. How do you roll back a database migration?](#q10-how-do-you-roll-back-a-database-migration)
  - [Q11. How do you structure Terraform state across accounts? DynamoDB lock or S3 lock file?](#q11-how-do-you-structure-terraform-state-across-accounts-dynamodb-lock-or-s3-lock-file)
  - [Q12. How do you detect and handle infrastructure drift?](#q12-how-do-you-detect-and-handle-infrastructure-drift)
  - [Q13. How do secrets flow at build time and at runtime? Where can they leak?](#q13-how-do-secrets-flow-at-build-time-and-at-runtime-where-can-they-leak)
  - [Q14. Why run CodeBuild inside a VPC? What does it cost you?](#q14-why-run-codebuild-inside-a-vpc-what-does-it-cost-you)
  - [Q15. How do you enforce separation of duties for HIPAA? Can one person ship to prod alone?](#q15-how-do-you-enforce-separation-of-duties-for-hipaa-can-one-person-ship-to-prod-alone)
  - [Q16. How do you measure DORA metrics, and what are your numbers?](#q16-how-do-you-measure-dora-metrics-and-what-are-your-numbers)
  - [Q17. The tooling account is compromised. What is the blast radius?](#q17-the-tooling-account-is-compromised-what-is-the-blast-radius)
  - [Q18. Why immutable tags? Why not use latest?](#q18-why-immutable-tags-why-not-use-latest)
  - [Q19. How would this scale to 4,000 deployments a day? What breaks first?](#q19-how-would-this-scale-to-4000-deployments-a-day-what-breaks-first)
  - [Q20. ECS now has built-in blue/green deployments. Why are you still on CodeDeploy?](#q20-ecs-now-has-built-in-bluegreen-deployments-why-are-you-still-on-codedeploy)
  - [Q21. Why separate AWS accounts for tooling, dev, staging and prod? Why not one account with IAM?](#q21-why-separate-aws-accounts-for-tooling-dev-staging-and-prod-why-not-one-account-with-iam)
  - [Q22. Why Terraform and not CloudFormation or CDK?](#q22-why-terraform-and-not-cloudformation-or-cdk)
- [Glossary](#glossary)
