...
- [5. Why each AWS service](#5-why-each-aws-service)
  - Services: [Amazon Route 53](#amazon-route-53) · [Amazon Application Recovery Controller (ARC) (old name Route 53 ARC)](#amazon-application-recovery-controller-arc-old-name-route-53-arc) · [AWS Certificate Manager (ACM)](#aws-certificate-manager-acm) · [Amazon CloudFront](#amazon-cloudfront) · [AWS WAF](#aws-waf) · [AWS Shield Advanced](#aws-shield-advanced) · [Amazon VPC (subnets)](#amazon-vpc-subnets) · [Internet Gateway](#internet-gateway) · [NAT Gateway](#nat-gateway) · [AWS Network Firewall](#aws-network-firewall) · [VPC Endpoints (Gateway and Interface)](#vpc-endpoints-gateway-and-interface) · [Application Load Balancer (ALB)](#application-load-balancer-alb) · [Amazon ECS on AWS Fargate](#amazon-ecs-on-aws-fargate) · [Amazon ElastiCache for Valkey](#amazon-elasticache-for-valkey) · [Amazon Aurora PostgreSQL (Global Database)](#amazon-aurora-postgresql-global-database) · [Amazon S3](#amazon-s3) · [AWS Backup](#aws-backup) · [AWS KMS (Key Management Service)](#aws-kms-key-management-service) · [AWS Secrets Manager](#aws-secrets-manager) · [AWS IAM (roles, permission boundaries, break-glass role)](#aws-iam-roles-permission-boundaries-break-glass-role) · [AWS Organizations (SCPs)](#aws-organizations-scps) · [AWS IAM Identity Center](#aws-iam-identity-center) · [Amazon CloudWatch](#amazon-cloudwatch) · [Amazon SNS](#amazon-sns) · [AWS CloudTrail](#aws-cloudtrail) · [Amazon VPC Flow Logs](#amazon-vpc-flow-logs) · [Amazon GuardDuty](#amazon-guardduty) · [Amazon Inspector](#amazon-inspector) · [Amazon Macie](#amazon-macie) · [AWS Config (conformance packs)](#aws-config-conformance-packs) · [AWS Security Hub](#aws-security-hub) · [Amazon EventBridge](#amazon-eventbridge) · [AWS Lambda](#aws-lambda)
  - [Key decisions](#key-decisions)
- [5A. Key topics](#5a-key-topics)
  - [Encryption (never leaving data in plain form anywhere)](#encryption-never-leaving-data-in-plain-form-anywhere)
  - [Key management (key hierarchy, rotation, separation of duties)](#key-management-key-hierarchy-rotation-separation-of-duties)
  - [Disaster recovery (Warm standby design)](#disaster-recovery-warm-standby-design)
  - [RTO / RPO (targets, and how each layer meets them)](#rto--rpo-targets-and-how-each-layer-meets-them)
  - [Audit logging (what, where, how long, how we prove it)](#audit-logging-what-where-how-long-how-we-prove-it)
  - [Least privilege (people, workloads, break-glass)](#least-privilege-people-workloads-break-glass)
  - [High availability to 99.99%](#high-availability-to-9999)
  - [Database failover (Multi-AZ failover, Global Database switchover vs failover)](#database-failover-multi-az-failover-global-database-switchover-vs-failover)
...
- [11. Failure scenarios](#11-failure-scenarios)
  - [Failure 1: Fargate task crash or bad deployment](#failure-1-fargate-task-crash-or-bad-deployment)
  - [Failure 2: A whole AZ is lost (or a gray failure)](#failure-2-a-whole-az-is-lost-or-a-gray-failure)
  - [Failure 3: Aurora writer instance failure](#failure-3-aurora-writer-instance-failure)
  - [Failure 4: The whole us-east-1 Region is lost](#failure-4-the-whole-us-east-1-region-is-lost)
  - [Failure 5: The ElastiCache Valkey primary is lost](#failure-5-the-elasticache-valkey-primary-is-lost)
  - [Failure 6: DDoS or credential stuffing attack](#failure-6-ddos-or-credential-stuffing-attack)
  - [Failure 7: Account compromise / ransomware attempt](#failure-7-account-compromise--ransomware-attempt)
  - [Failure 8: Partner / card network cannot be reached](#failure-8-partner--card-network-cannot-be-reached)
  - [Failure 9: A KMS key is disabled by mistake](#failure-9-a-kms-key-is-disabled-by-mistake)
...
- [14. Deep-dive questions and answers](#14-deep-dive-questions-and-answers)
  - [Q1. Why did you choose ECS on Fargate instead of EKS for a bank?](#q1-why-did-you-choose-ecs-on-fargate-instead-of-eks-for-a-bank)
  - [Q2. Walk me through an Aurora writer failure. What does the application actually see?](#q2-walk-me-through-an-aurora-writer-failure-what-does-the-application-actually-see)
  - [Q3. Explain Aurora Global Database switchover versus failover, with timelines and data loss.](#q3-explain-aurora-global-database-switchover-versus-failover-with-timelines-and-data-loss)
  - [Q4. How exactly do you fail over to us-west-2, and why is it not automatic?](#q4-how-exactly-do-you-fail-over-to-us-west-2-and-why-is-it-not-automatic)
  - [Q5. Is 99.99% realistic here? How do you actually get there?](#q5-is-9999-realistic-here-how-do-you-actually-get-there)
  - [Q6. Why Aurora PostgreSQL instead of DynamoDB, or plain RDS for PostgreSQL?](#q6-why-aurora-postgresql-instead-of-dynamodb-or-plain-rds-for-postgresql)
  - [Q7. How do you protect card numbers and SSNs beyond disk encryption?](#q7-how-do-you-protect-card-numbers-and-ssns-beyond-disk-encryption)
  - [Q8. Walk me through your KMS key strategy. Who can do what with a key?](#q8-walk-me-through-your-kms-key-strategy-who-can-do-what-with-a-key)
  - [Q9. Why multi-Region KMS keys? Does Aurora need them?](#q9-why-multi-region-kms-keys-does-aurora-need-them)
  - [Q10. How do you prove to an auditor that audit logs were not tampered with?](#q10-how-do-you-prove-to-an-auditor-that-audit-logs-were-not-tampered-with)
  - [Q11. An attacker gets admin in the production account. What can they still destroy?](#q11-an-attacker-gets-admin-in-the-production-account-what-can-they-still-destroy)
  - [Q12. Everyone is read-only by default. How does an engineer fix production at 3 AM?](#q12-everyone-is-read-only-by-default-how-does-an-engineer-fix-production-at-3-am)
  - [Q13. Why put CloudFront in front of an API you never cache? And why not a private ALB with CloudFront VPC origins?](#q13-why-put-cloudfront-in-front-of-an-api-you-never-cache-and-why-not-a-private-alb-with-cloudfront-vpc-origins)
  - [Q14. How does your egress control work? Why Network Firewall, not just security groups?](#q14-how-does-your-egress-control-work-why-network-firewall-not-just-security-groups)
  - [Q15. How do database credentials rotate without breaking 3,000 req/s?](#q15-how-do-database-credentials-rotate-without-breaking-3000-reqs)
  - [Q16. The client times out and retries a transfer. How do you make sure money moves only once?](#q16-the-client-times-out-and-retries-a-transfer-how-do-you-make-sure-money-moves-only-once)
  - [Q17. Traffic suddenly grows 10x (for example, after a big employer partnership). What breaks first?](#q17-traffic-suddenly-grows-10x-for-example-after-a-big-employer-partnership-what-breaks-first)
  - [Q18. How do you test DR, and what did the drills teach you?](#q18-how-do-you-test-dr-and-what-did-the-drills-teach-you)
  - [Q19. Why warm standby and not active-active across two regions?](#q19-why-warm-standby-and-not-active-active-across-two-regions)
  - [Q20. Shield Advanced costs about $3,000 a month. How do you justify it?](#q20-shield-advanced-costs-about-3000-a-month-how-do-you-justify-it)
  - [Q21. Why did you split security, logging and backups into separate AWS accounts?](#q21-why-did-you-split-security-logging-and-backups-into-separate-aws-accounts)
- [Glossary](#glossary)
