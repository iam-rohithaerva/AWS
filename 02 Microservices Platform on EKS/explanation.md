...
- [5. Why each AWS service](#5-why-each-aws-service)
  - Services: [Amazon Route 53](#amazon-route-53) · [Amazon CloudFront](#amazon-cloudfront) · [AWS WAF](#aws-waf) · [AWS Certificate Manager (ACM)](#aws-certificate-manager-acm) · [AWS Shield Standard](#aws-shield-standard) · [Application Load Balancer (with AWS Load Balancer Controller)](#application-load-balancer-with-aws-load-balancer-controller) · [Amazon VPC (subnets, Internet Gateway)](#amazon-vpc-subnets-internet-gateway) · [NAT Gateway](#nat-gateway) · [VPC Endpoints](#vpc-endpoints) · [Amazon EKS (control plane)](#amazon-eks-control-plane) · [EKS Managed Node Groups (EC2 system nodes)](#eks-managed-node-groups-ec2-system-nodes) · [Karpenter nodes (Amazon EC2, Graviton and Spot)](#karpenter-nodes-amazon-ec2-graviton-and-spot) · [Amazon ECR](#amazon-ecr) · [Amazon RDS for PostgreSQL (Multi-AZ DB instance + read replica)](#amazon-rds-for-postgresql-multi-az-db-instance--read-replica) · [Amazon ElastiCache for Valkey](#amazon-elasticache-for-valkey) · [Amazon SQS (with DLQ)](#amazon-sqs-with-dlq) · [Amazon S3](#amazon-s3) · [AWS Secrets Manager](#aws-secrets-manager) · [AWS IAM (EKS Pod Identity and IRSA)](#aws-iam-eks-pod-identity-and-irsa) · [AWS KMS](#aws-kms) · [Amazon CloudWatch (Container Insights, Logs)](#amazon-cloudwatch-container-insights-logs) · [AWS CloudTrail](#aws-cloudtrail) · [Amazon SNS](#amazon-sns) · [Amazon Managed Service for Prometheus](#amazon-managed-service-for-prometheus) · [Amazon Managed Grafana](#amazon-managed-grafana) · [AWS Backup](#aws-backup)
  - [Key decisions](#key-decisions)
- [5A. Key topics](#5a-key-topics)
  - [Why EKS instead of ECS](#why-eks-instead-of-ecs)
  - [Multi-tenancy for 10 teams](#multi-tenancy-for-10-teams)
  - [Kubernetes Services](#kubernetes-services)
  - [Pod-level IAM: IRSA vs EKS Pod Identity](#pod-level-iam-irsa-vs-eks-pod-identity)
  - [Pod and node autoscaling end to end](#pod-and-node-autoscaling-end-to-end)
  - [EKS cluster upgrade strategy with zero downtime](#eks-cluster-upgrade-strategy-with-zero-downtime)
  - [IP address planning for EKS](#ip-address-planning-for-eks)
...
- [11. Failure scenarios](#11-failure-scenarios)
  - [Failure 1: Spot node interruption (losing one worker node)](#failure-1-spot-node-interruption-losing-one-worker-node)
  - [Failure 2: Availability Zone failure (losing us-west-2a)](#failure-2-availability-zone-failure-losing-us-west-2a)
  - [Failure 3: RDS writer failure (shipments DB)](#failure-3-rds-writer-failure-shipments-db)
  - [Failure 4: Bad deploy, pods CrashLoopBackOff](#failure-4-bad-deploy-pods-crashloopbackoff)
  - [Failure 5: SQS backlog, poison message](#failure-5-sqs-backlog-poison-message)
  - [Failure 6: Pod IP exhaustion (pods stuck in ContainerCreating)](#failure-6-pod-ip-exhaustion-pods-stuck-in-containercreating)
  - [Failure 7: Cluster-wide issue after upgrade (admission webhook down)](#failure-7-cluster-wide-issue-after-upgrade-admission-webhook-down)
  - [Failure 8: Region failure (all of us-west-2)](#failure-8-region-failure-all-of-us-west-2)
...
- [14. Deep-dive questions and answers](#14-deep-dive-questions-and-answers)
  - [Q1. Why did you choose EKS instead of ECS?](#q1-why-did-you-choose-eks-instead-of-ecs)
  - [Q2. How does a pod get AWS permissions? IRSA or Pod Identity?](#q2-how-does-a-pod-get-aws-permissions-irsa-or-pod-identity)
  - [Q3. Why must the node IAM role stay minimal, and how do you stop pods from using it?](#q3-why-must-the-node-iam-role-stay-minimal-and-how-do-you-stop-pods-from-using-it)
  - [Q4. How does a request get from the ALB to a pod? Why target type ip?](#q4-how-does-a-request-get-from-the-alb-to-a-pod-why-target-type-ip)
  - [Q5. How do you avoid 5xx errors during rolling deployments?](#q5-how-do-you-avoid-5xx-errors-during-rolling-deployments)
  - [Q6. How do you upgrade EKS with zero downtime?](#q6-how-do-you-upgrade-eks-with-zero-downtime)
  - [Q7. Traffic suddenly goes 10x. What happens, layer by layer?](#q7-traffic-suddenly-goes-10x-what-happens-layer-by-layer)
  - [Q8. Karpenter or Cluster Autoscaler? Why?](#q8-karpenter-or-cluster-autoscaler-why)
  - [Q9. How did you plan IP addresses? What if pods run out of IPs?](#q9-how-did-you-plan-ip-addresses-what-if-pods-run-out-of-ips)
  - [Q10. How do you isolate 10 teams on one cluster? Why not one cluster per team?](#q10-how-do-you-isolate-10-teams-on-one-cluster-why-not-one-cluster-per-team)
  - [Q11. How do secrets reach the pods, and how are they protected?](#q11-how-do-secrets-reach-the-pods-and-how-are-they-protected)
  - [Q12. Why RDS PostgreSQL, and why one database per business area?](#q12-why-rds-postgresql-and-why-one-database-per-business-area)
  - [Q13. 900 pods talk to RDS. How do you avoid running out of connections?](#q13-900-pods-talk-to-rds-how-do-you-avoid-running-out-of-connections)
  - [Q14. Why SQS with KEDA? How do you handle retries and duplicates?](#q14-why-sqs-with-keda-how-do-you-handle-retries-and-duplicates)
  - [Q15. Why CloudFront in front of the ALB, and how is the ALB locked down?](#q15-why-cloudfront-in-front-of-the-alb-and-how-is-the-alb-locked-down)
  - [Q16. An Availability Zone goes down. Walk me through what happens.](#q16-an-availability-zone-goes-down-walk-me-through-what-happens)
  - [Q17. Your DR is backup and restore with a 4-hour RTO. Why not warm standby?](#q17-your-dr-is-backup-and-restore-with-a-4-hour-rto-why-not-warm-standby)
  - [Q18. A pod is stuck in Pending or CrashLoopBackOff. How do you debug it?](#q18-a-pod-is-stuck-in-pending-or-crashloopbackoff-how-do-you-debug-it)
  - [Q19. How does one microservice call another inside the cluster?](#q19-how-does-one-microservice-call-another-inside-the-cluster)
- [Glossary](#glossary)
