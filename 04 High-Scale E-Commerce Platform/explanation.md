...
- [5. Why each AWS service](#5-why-each-aws-service)
  - Services: [Amazon Route 53](#amazon-route-53) · [Amazon CloudFront](#amazon-cloudfront) · [AWS WAF](#aws-waf) · [AWS Shield Advanced](#aws-shield-advanced) · [AWS Lambda (Waiting room + Notify worker)](#aws-lambda-waiting-room--notify-worker) · [Amazon Cognito](#amazon-cognito) · [Amazon VPC (subnets, Internet Gateway)](#amazon-vpc-subnets-internet-gateway) · [NAT Gateway](#nat-gateway) · [VPC Endpoints](#vpc-endpoints) · [Application Load Balancer (ALB)](#application-load-balancer-alb) · [Amazon ECS (Fargate, Graviton EC2, Fargate Spot)](#amazon-ecs-fargate-graviton-ec2-fargate-spot) · [Amazon ECR](#amazon-ecr) · [AWS AppConfig](#aws-appconfig) · [Amazon Aurora PostgreSQL (catalog, orders)](#amazon-aurora-postgresql-catalog-orders) · [Amazon RDS Proxy](#amazon-rds-proxy) · [Amazon DynamoDB (carts, inventory)](#amazon-dynamodb-carts-inventory) · [Amazon ElastiCache for Valkey](#amazon-elasticache-for-valkey) · [Amazon OpenSearch Service](#amazon-opensearch-service) · [Amazon S3 (product-media)](#amazon-s3-product-media) · [Amazon EventBridge](#amazon-eventbridge) · [AWS Step Functions (order saga)](#aws-step-functions-order-saga) · [Amazon SNS](#amazon-sns) · [Amazon SQS](#amazon-sqs) · [Amazon SES](#amazon-ses) · [AWS End User Messaging (SMS)](#aws-end-user-messaging-sms) · [AWS IAM (task roles, execution roles)](#aws-iam-task-roles-execution-roles) · [AWS KMS](#aws-kms) · [AWS Secrets Manager](#aws-secrets-manager) · [Amazon CloudWatch](#amazon-cloudwatch) · [AWS X-Ray (via OpenTelemetry)](#aws-x-ray-via-opentelemetry) · [AWS CloudTrail](#aws-cloudtrail) · [AWS Backup](#aws-backup)
  - [Key decisions](#key-decisions)
- [5A. Key topics](#5a-key-topics)
  - [Domain by domain (how we handled each domain)](#domain-by-domain-how-we-handled-each-domain)
  - [Synchronous vs event-driven (what is sync, what is async)](#synchronous-vs-event-driven-what-is-sync-what-is-async)
  - [Black Friday readiness (getting ready in advance)](#black-friday-readiness-getting-ready-in-advance)
  - [Inventory overselling and hot keys (not selling too much, the hot SKU problem)](#inventory-overselling-and-hot-keys-not-selling-too-much-the-hot-sku-problem)
  - [Payment idempotency and the outbox pattern](#payment-idempotency-and-the-outbox-pattern)
  - [Why ECS instead of EKS (why ECS for this organization)](#why-ecs-instead-of-eks-why-ecs-for-this-organization)
...
- [11. Failure scenarios](#11-failure-scenarios)
  - [Failure 1: ECS task crash (application crash, memory leak)](#failure-1-ecs-task-crash-application-crash-memory-leak)
  - [Failure 2: A whole AZ is lost (AZ failure)](#failure-2-a-whole-az-is-lost-az-failure)
  - [Failure 3: The Aurora orders writer is lost (database failure)](#failure-3-the-aurora-orders-writer-is-lost-database-failure)
  - [Failure 4: Hot SKU throttling in a flash sale](#failure-4-hot-sku-throttling-in-a-flash-sale)
  - [Failure 5: Payment provider slow or down (external dependency)](#failure-5-payment-provider-slow-or-down-external-dependency)
  - [Failure 6: Queue backlog and DLQ (notification failure)](#failure-6-queue-backlog-and-dlq-notification-failure)
  - [Failure 7: Cache failure and Aurora stampede](#failure-7-cache-failure-and-aurora-stampede)
  - [Failure 8: The outbox relay slows down (Fargate Spot interruption)](#failure-8-the-outbox-relay-slows-down-fargate-spot-interruption)
  - [Failure 9: A whole region is lost (region failure)](#failure-9-a-whole-region-is-lost-region-failure)
...
- [14. Deep-dive questions and answers](#14-deep-dive-questions-and-answers)
  - [Q1. Walk me through checkout. Which parts are synchronous and which are event-driven, and why?](#q1-walk-me-through-checkout-which-parts-are-synchronous-and-which-are-event-driven-and-why)
  - [Q2. Why ECS instead of EKS for this company?](#q2-why-ecs-instead-of-eks-for-this-company)
  - [Q3. Two shoppers click "buy" on the last unit at the same moment. What happens?](#q3-two-shoppers-click-buy-on-the-last-unit-at-the-same-moment-what-happens)
  - [Q4. How do you handle a hot SKU that gets thousands of writes per second?](#q4-how-do-you-handle-a-hot-sku-that-gets-thousands-of-writes-per-second)
  - [Q5. The payment call timed out. How do you make sure the customer is not charged twice?](#q5-the-payment-call-timed-out-how-do-you-make-sure-the-customer-is-not-charged-twice)
  - [Q6. What is the transactional outbox, and why not publish to EventBridge right after the database commit?](#q6-what-is-the-transactional-outbox-and-why-not-publish-to-eventbridge-right-after-the-database-commit)
  - [Q7. Why Aurora for orders but DynamoDB for cart and inventory?](#q7-why-aurora-for-orders-but-dynamodb-for-cart-and-inventory)
  - [Q8. Why RDS Proxy? What problems can it cause?](#q8-why-rds-proxy-what-problems-can-it-cause)
  - [Q9. Why put CloudFront in front of the ALB, and how do you stop people from hitting the ALB directly?](#q9-why-put-cloudfront-in-front-of-the-alb-and-how-do-you-stop-people-from-hitting-the-alb-directly)
  - [Q10. How does your virtual waiting room work?](#q10-how-does-your-virtual-waiting-room-work)
  - [Q11. Why EventBridge, then SNS, then SQS for notifications? Why not SQS instead of EventBridge?](#q11-why-eventbridge-then-sns-then-sqs-for-notifications-why-not-sqs-instead-of-eventbridge)
  - [Q12. Why Step Functions for the order saga instead of pure EventBridge choreography?](#q12-why-step-functions-for-the-order-saga-instead-of-pure-eventbridge-choreography)
  - [Q13. How did you prepare for Black Friday? Which quotas did you raise?](#q13-how-did-you-prepare-for-black-friday-which-quotas-did-you-raise)
  - [Q14. The payment provider is slow on Black Friday. What happens to your system?](#q14-the-payment-provider-is-slow-on-black-friday-what-happens-to-your-system)
  - [Q15. How do you keep the cache, OpenSearch and CloudFront consistent with Aurora when a price changes?](#q15-how-do-you-keep-the-cache-opensearch-and-cloudfront-consistent-with-aurora-when-a-price-changes)
  - [Q16. Is Shield Advanced worth its monthly fee? Would you buy it again?](#q16-is-shield-advanced-worth-its-monthly-fee-would-you-buy-it-again)
  - [Q17. Why mix Fargate, Graviton EC2 and Fargate Spot? Isn't that extra complexity?](#q17-why-mix-fargate-graviton-ec2-and-fargate-spot-isnt-that-extra-complexity)
  - [Q18. What happens if all of us-east-1 goes down? Why not multi-region?](#q18-what-happens-if-all-of-us-east-1-goes-down-why-not-multi-region)
  - [Q19. How do services authenticate users, and how do services call each other securely?](#q19-how-do-services-authenticate-users-and-how-do-services-call-each-other-securely)
  - [Q20. Why a NAT gateway in every AZ, and why so many VPC endpoints?](#q20-why-a-nat-gateway-in-every-az-and-why-so-many-vpc-endpoints)
  - [Q21. Why Aurora instead of plain RDS PostgreSQL?](#q21-why-aurora-instead-of-plain-rds-postgresql)
  - [Q22. Why ElastiCache if you already have CloudFront and Aurora readers?](#q22-why-elasticache-if-you-already-have-cloudfront-and-aurora-readers)
- [Glossary](#glossary)
